...
import rehypeParse from './rehype-parse.js'
...
test('rehype', async function (t) {
  await t.test('should expose the public api of `rehype`', async function () {
    assert.deepEqual(Object.keys(await import('./rehype.js')).sort(), ['rehype'])
  })

  await t.test(
    'should expose the public api of `rehype-parse`',
    async function () {
      assert.deepEqual(Object.keys(await import('./rehype-parse.js')).sort(), [
        'default'
      ])
    }
  )
  ...
  await t.test(
    ...
    async function () {
      assert.deepEqual(Object.keys(await import('./rehype-stringify.js')).sort(), [
        'default'
      ])
    }
  )
